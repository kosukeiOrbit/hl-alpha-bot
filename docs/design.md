...
#### REGIME 層入力の BTC 共通 / 銘柄個別 内訳（PR7.6.7 追記）

REGIME 層は名称が「マクロフィルター」で BTC 一本に見えるが、実際の入力は
**BTC 共通項目と評価対象銘柄個別項目が混在**している:

| 入力 | データソース | 銘柄ごとに変わる？ |
|---|---|---|
| `btc_ema_trend` | BTC のみ（`get_market_snapshot("BTC")` で別取得） | ❌ 全銘柄共通 |
| `btc_atr_pct` | BTC のみ（同上） | ❌ 全銘柄共通 |
| `funding_rate` | 評価対象銘柄 | ✅ 銘柄ごと |
| `oi_change_1h_pct` | 評価対象銘柄 | ✅ 銘柄ごと |

`entry_judge.py` の SHORT REGIME は具体的に:

```python
(snap.btc_ema_trend != "UPTREND"
 OR snap.funding_rate > _SHORT_FUNDING_RATE_OVERHEATED)  # 0.03%/8h
AND abs(snap.oi_change_1h_pct) < _LONG_OI_CHANGE_MAX_PCT
```

→ **`btc_ema_trend` がメインゲート**。BTC が UPTREND の時、評価銘柄の
...
### 7.11 PR7.5e-1 実装で確定した FundingRateSentimentProvider 仕様

Claude API 不使用・コスト $0 の SentimentProvider 第一弾。
HL の funding rate を **contrarian view（逆張り視点）** で sentiment に変換する。

**論理:**
- Funding > 0（過剰 LONG） → 巻き戻しリスク → sentiment は bearish 寄り
- Funding < 0（過剰 SHORT） → ショートカバー余地 → sentiment は bullish 寄り
- つまり `score = -funding_rate_8h * scale_factor`（符号反転）

**実装:**

```python
# src/infrastructure/funding_rate_sentiment_provider.py（要約）
async def _compute(self, symbol: str, cached: bool) -> SentimentResult:
    ...
    return SentimentResult(
        score=score, confidence=self.config.confidence,
        direction=sentiment_dir, ...,
    )
```

**CORE 閾値（章11.4 entry_judge）との整合:**
- LONG 通過に必要: `score > 0.6` → `funding_rate_8h < -0.0006`（scale=1000 時）
- SHORT 通過に必要: `score < -0.3` → `funding_rate_8h > +0.0003`（scale=1000 時）

**確定した実装挙動（テストで検証済み）:**
- 境界は **strict 不等式**: `score == 0.6` ちょうどは **neutral**（>0.6 にしない）
- 同じく `score == -0.3` ちょうども neutral
- `judge` は cache を**読まないし書かない**（fresh 計算のみ）
- `judge_cached_or_fresh` だけが cache を経由する
- 取得失敗（ExchangeError）時は `SentimentError` に変換、**cache を残さない**
  → 次回 fresh 取得が必ず走る
- `fetch_sources` は空 tuple を返す（funding ベースなのでニュースソース不要）

**設定（章23.x SentimentSettings）:**
```python
class SentimentSettings(BaseModel):
    ...
    funding_scale_factor: Decimal = Decimal("10000")
    funding_cache_window_seconds: int = 300  # funding は 1h 単位なので 5 分で十分
    funding_confidence: Decimal = Decimal("0.8")  # 固定（funding 自体は安定指標）
```

`provider` 既定 `"fixed"` で後方互換維持。Phase 1 移行は `profile_phase1.yaml`
で `provider: "funding_rate"` を指定するだけ。
...
def _build_sentiment_provider(
    ...
    if settings.sentiment.provider == "funding_rate":
        return FundingRateSentimentProvider(exchange, FundingRateSentimentConfig(
            scale_factor=settings.sentiment.funding_scale_factor,
            cache_window_seconds=settings.sentiment.funding_cache_window_seconds,
...
将来 PR7.5e-2 で `ClaudeSentimentProvider` を、PR7.5e-3 で複数 Provider 合成
（`CompositeSentimentProvider`）を追加する余地を残す。
...
#### 9.15.5 BreakerInput の実値計算（PR7.4-real）

PR7.4 で `weekly_loss_pct=0` / `symbol_1min_changes_pct=()` /
`btc_5min_change_pct=0` / `api_error_rate_5min=0` の placeholder で埋めて
いた 4 つの指標を、PR7.4-real ですべて実値計算に置換。本番運用で章9.7 の
全 7 Layer ブレーカーが機能可能になった。

**Layer 別の実値ソース:**

| Layer | 指標 | 取得方法 |
|---|---|---|
| 1 | `daily_loss_pct` | `daily_pnl / balance * 100`（既存・PR7.4） |
| 2 | `weekly_loss_pct` | `Repository.get_pnl_since(now-7d) / balance * 100`（新規） |
| 3 | `consecutive_losses` | `Repository.get_consecutive_losses()`（既存） |
| 4 | `symbol_1min_changes_pct` | watchlist 各銘柄の 1m × 2 本から `(c1-c0)/c0*100`（新規） |
| 5 | `btc_5min_change_pct` | BTC 5m × 2 本から同様（新規） |
| 6 | `api_error_rate_5min` | `ApiCallTracker.error_rate()`（新規・後述） |
| 7 | `position_count` | `exchange.get_positions()` の長さ（既存） |

**新規 Repository メソッド `get_pnl_since`:**
```python
async def get_pnl_since(self, since: datetime) -> Decimal:
    """since 以降に exit した実弾 trades の累計 PnL（章9.7 Layer 2）。"""
    # is_dry_run=0 のみ集計。tzinfo なしは UTC として扱う。
```

**ApiCallTracker（章9.7 Layer 6 入力）:**

cycle 単位の成功/失敗を 5 分 rolling window で in-memory に記録。
全 API コールをラップする per-call tracking は大改修なので、PR7.4-real では
**cycle 粒度（1 cycle = 1 サンプル）** を採用：
...
class ApiCallTracker:
    ...
    min_samples: int = 10  # サンプル < 10 なら 0 を返す（サンプル不足を発火扱いしない）
...
`Scheduler.run()` の cycle ループに記録を埋め込み：
```python
...
BOT 再起動でリセットされる（in-memory のみ）が、5 分 window なので 5 分稼働
すれば再構築される。永続化は不要。

**並行取得（asyncio.gather）で cycle 時間短縮:**

`_build_breaker_input` 内で独立した取得は並行化：
```python
positions, consecutive, daily_pnl, weekly_pnl, symbol_changes, btc_5min_change = (
...
順次なら +600ms 程度かかる API コール群が ~200ms に圧縮される（実測）。

**個別 try/except で部分故障を隔離:**

`_compute_symbol_1min_changes_pct` は symbol 単位で try/except し、
失敗銘柄を結果から除外（他銘柄の判定は維持）。`_compute_btc_5min_change_pct`
は失敗時 0 を返す（Layer 5 は発動しない＝安全側）。

`_build_breaker_input` 全体の try/except（fail-open）も維持しているので、
多層防御で「1 指標取得失敗で全 Layer 無効化」を防ぐ。

**Phase 0 / Phase 1 (dry_run=true) での挙動:**

trades テーブルに実弾レコードが入らないため Layer 1-3（loss-based）は
常に 0 で発動しない（仕様通り）。Layer 4-7 は dry_run でも動作するので、
価格急変ブレーカー（Layer 4-5）と API/ポジション系（Layer 6-7）は
Phase 0 / 1 でも実機検証可能。

#### 9.15.6 CircuitBreakerSettings は SchedulerConfig に統合済み

spec 想定の独立した `CircuitBreakerSettings` クラスは作らず、既存の
`SchedulerConfig` に閾値を統合する方針。理由：

- 既に `SchedulerConfig` が `daily_loss_limit_pct` / `weekly_loss_limit_pct` /
  `consecutive_loss_limit` / `flash_crash_threshold_pct` /
  `btc_anomaly_threshold_pct` / `api_error_rate_max` /
  `position_overflow_multiplier` / `max_position_count` をすべて保持
- Scheduler 起動時に DI（`SchedulerConfig` 経由）でブレーカー閾値が渡る構造
- 設定セクションを増やすメリットがない（pydantic schema もそのまま）

将来 CB 専用設定（例: 各 Layer ごとの enable/disable）が増えれば独立クラス
に分離する余地はある。PR7.4-real 時点では `SchedulerConfig` で十分。
...
### 11.19 PR 3df8bea + c9b2d50 で確定した silent rejection 可視化

mainnet Phase 2 Step 2b 初日（2026-05-13）に「ETH SHORT 4 層通過 5 件 →
trades 0 件 / incidents 0 件 / ログ無し」事象が発生。原因切り分けで
2 つの独立した問題が判明:

1. unified account 未対応で balance=$0 と認識（章22.7 Unified Account 節）
2. `place_orders_grouped` が entry slot inner error を silent return
   する非対称性（本節）

#### 11.19.1 place_order / place_orders_grouped の非対称性（PR c9b2d50 で統一）

実機検証（手動 ALO テスト・2026-05-14）で確定:

| シナリオ | `place_order`（単発） | `place_orders_grouped`（修正前） |
|---|---|---|
| Stage A: markPx × 1.15 sell ALO（板はるか上） | success=True, order_id 取得 | success=True, order_id 取得 |
| Stage B: markPx 近傍 sell ALO | **raise `OrderRejectedError(code="ALO_REJECT")`** | **`OrderResult(success=False, rejected_reason="Post only order would have immediately matched, bbo was X@Y. asset=N")`** を silent return |

同一拒否事由（HL の `statuses[0].error`）が、単発経路では例外として表面化
する一方、grouped 経路では `OrderResult(success=False)` で返却され
`entry_flow` の既存例外 catch を通らず**完全に黙殺**されていた。

**PR c9b2d50 の修正（Level 3 構造的統一）:**

`_grouped_status_to_result` を **idx==0（entry slot）のみ `_raise_inner_error`
経由**に変更:
...
def _grouped_status_to_result(idx: int, status: Any) -> OrderResult:
    if "error" in status:
        ...
        if idx == 0:
            # entry slot: 単発 place_order と同じ raise 経路
            HyperLiquidClient._raise_inner_error(error_msg)
        # TP/SL slot (idx>=1) は raise しない:
        # entry が既に resting/filled で成功している partial success
        # ケースで raise すると tuple ごと失われ、dangling entry order
        # （TP/SL なしで放置された注文）が発生する。mainnet で重大事故。
...
- 単純に全 inner error で raise すると、`statuses[0]` で entry が
  `resting`/`filled` として成功している partial success ケースで
  tuple そのものが失われる
- TP/SL slot の error は `OrderResult(success=False)` で返して
  partial success 情報を保つ
- `_execute_entry` 側で TP/SL の `success=False` を個別判定する
  余地を残す（PR 範囲外・将来課題）

#### 11.19.2 silent return 経路の可視化（PR 3df8bea / Level 1+2）

PR c9b2d50 と組み合わせて、`_execute_entry` の残る silent return point
2 箇所にも `logger.warning` + `notifier.send_alert` を追加:

- `sizing.size_coins <= 0` 経路 → `dedup_key="entry_skip_size:{symbol}:{direction}"`
- grouped 結果が `success=False` または `order_id=None` 経路
  → `dedup_key="entry_skip_reject:{symbol}:{direction}"`

挙動変更ゼロ（return 値は同じ）、観察可能性のみ追加。

#### 11.19.3 統合後の rejection 可視化マトリクス

mainnet 4 層通過 → grouped 発注時の挙動（PR 3df8bea + c9b2d50 適用後）:

| シナリオ | 経路 | log + alert |
|---|---|---|
| 残高十分・約定 | trades に記録 | `entry:{trade_id}` signal |
| ALO 拒否（entry slot） | exception → catch | `entry_fail:{symbol}:{direction}` alert |
| TP/SL 拒否（partial success） | tuple 返却 → defensive check | `entry_skip_reject:{symbol}:{direction}` alert |
| `size_too_small`（balance 不足等） | sizing<=0 → silent skip | `entry_skip_size:{symbol}:{direction}` alert |
| 空 results / `success=True` + `order_id=None` | defensive check | `entry_skip_reject:{symbol}:{direction}` alert |
| ~~silent return（黙殺）~~ | **発生しなくなる** | — |

3 つの `dedup_key` で「例外で落ちた / size が小さい / HL に拒否された」
の 3 状態が Discord で識別可能。`entry_fail` と `entry_skip_reject` の
二重通知にはならない（ALO 拒否は exception path に流れる、defensive
check には到達しない）。
...
#### 15.4.5 PR7.5e-1 効果検証: Phase 1 FundingRate 観察（testnet 48h）

`profile_phase1.yaml`（`provider: "funding_rate"`）に切替後、約 48 時間の
testnet 観察。`scale_factor=1000` への手動チューニング後の計測値：

```
Phase 1 期間（PR7.5e-1・48h）の各層通過率:
  MOMENTUM:   1.4%   ← Phase 0 (PR7.7 / 72h Fixed) の 2.0% から低下
  SENTIMENT:  0.1%   ← Phase 0 forced bullish の 100.0% から大幅低下
  REGIME:    58.8%   ← Phase 0 (PR7.7) の 77.7% から低下（市場依存）
  FLOW:       100%   ← 簡易実装で常時 1（PR7.x 検討中）

4 層 AND 通過:
  Phase 0 (PR7.7 / 72h・FixedSentiment): 452 件
  Phase 1 (FundingRate / 48h):             0 件

直近 sentiment_score 分布（200 サンプル）:
  全サンプルが negative 領域（min -1.0 / max -0.1 / avg -0.55）
  bullish (>0.6): 0 件
  bearish (<-0.3): ~50%
  neutral (-0.3 〜 0.6): ~50%
  → testnet で funding が常時 positive（過剰 LONG 常態化）
  → contrarian view では永遠に LONG にならない判定が出続ける
```

**結論: PR7.5e-1 は設計通り機能している**

4 層 AND 通過 0 は「シグナル品質悪化」ではなく「**過剰 LONG では LONG しない**」
という正しい判断。SENTIMENT 0.1% は contrarian view が「現状で LONG を打つな」
と言っている状態であって、Provider のバグではない。

#### 15.4.6 scale_factor チューニング履歴

```
初期実装: scale_factor=10000
  funding 0.0001 で score=±1.0 に飽和。
  testnet では funding が常に ~0.0001 以上なので score は常に -1.0 で
  bearish 領域に張り付き続けた（観察 48h で一度も positive に到達せず）。

PR7.5e-1-fix (commit da6edc7): scale_factor=1000
  funding 0.0001 で score=±0.1。中立帯を広げる意図。
  しかし testnet では funding 0.001 以上が常態化していたため、
  scale=1000 でも結局 -1.0 にクリップされ続けた（SENTIMENT 通過率 6% → 0.1%）。
  むしろ強い bearish 判定として contrarian view が正しく機能している証拠。

根本原因:
  testnet の市場特性（流動性低・bot 集団行動による funding 偏り）が
  本質的な原因。scale_factor だけでは解決しない。
  mainnet 移行後に再評価する。
```

#### 15.4.7 testnet dead market の 3 軸独立分析

testnet 1 週間以上の観察で判明した構造的問題。**3 つの独立した軸**で
LONG 不適シグナルが同時に出続けるため 4 層 AND が成立しない：

| 軸 | 指標 | 観察値 | LONG への影響 |
|---|---|---|---|
| トレンド | REGIME (EMA20 vs EMA50) | 12-58% に揺らぐ・BTC DOWNTREND の瞬間が多い | ✗ |
| 動き | MOMENTUM (5bar % change) | 0-2%・dead market | ✗ |
| 偏り | SENTIMENT (funding contrarian) | 0-6%・常に bearish 側 | ✗ |

3 軸が同時に好転する瞬間がほぼ無い → 4 層 AND 0 件が継続。
これは設計上 **「不適切な相場で実弾を打たない」フィルタが機能している証拠**
だが、**観察データが取れない別問題**を生む。

**mainnet 移行の必要性:**

testnet の市場特性（実需要が薄い・bot 集団 funding 偏り）では Phase 1
sentiment の現実的な評価が不可能。実トレーダーが参加する mainnet で
再評価する必要がある（少額 5 USDC + Arbitrum ETH 入金で開始可能）。
別 PR で `profile_phase2.yaml` 検討。

#### 15.4.8 SHORT 検証用プロファイル（profile_phase1_short.yaml）

mainnet 移行前の SHORT 実装の実機統合テスト用に追加。`dry_run=true` 維持
なので実弾リスクなし：

```yaml
# config/profile_phase1_short.yaml の要点
phase: phase_1
trading:
  is_dry_run: true          # SHORT は無限損失リスクのため特に重要
watchlist:
  directions: [LONG, SHORT]  # ★ SHORT 追加
sentiment:
  provider: "funding_rate"
  funding_scale_factor: 1000
scheduler:
  cycle_interval_seconds: 20  # ★ 429 対策（詳細は章19.3）
```

**観察目的:**
- attempts が 2 → 4 に増加（BTC LONG/SHORT + ETH LONG/SHORT）
- SHORT 方向の signals が DB に正しく記録されること
- testnet bearish 領域で SHORT 4 層 AND が成立するか
- mainnet 移行前の SHORT 経路バリデーション

最終的な 4 層通過件数や issue は次回の設計書集約 PR で記録する。

#### 15.4.9 Phase 2 Step 2a/2b: mainnet 移行の実施記録（2026-05-13）

testnet 1 週間の dead-market 観察を踏まえ、Phase 1 sentiment の現実的な
評価には mainnet が必要と判断（章15.4.7）。Phase 2 Step 2a/2b として
段階的に移行した。

**Phase 2 Step 2a — mainnet 接続 + dry_run 観察:**
- `config/profile_phase2.yaml` 新規作成（`exchange.network: mainnet` /
  `trading.is_dry_run: true` で開始）
- mainnet Agent Wallet 生成（HL UI の API ページ）
  - Agent name: `hl-bot-mainnet`
  - Master Wallet は testnet と共用（同一の master_address）
  - 有効期限あり（生成時に指定）
- `secrets.enc.yaml` を mainnet 用に切替
  - testnet 版は `secrets.enc.testnet.yaml` としてバックアップ
  - フラット構造を維持（`hyperliquid: master_address /
    agent_private_key / agent_address / network`）
- `verify_phase0.py` で mainnet 接続確認（errors=0）

**Phase 2 Step 2b — 実弾投入:**
- `trading.is_dry_run: false` に変更
- `trading.position_size_pct: 0.005 → 0.02` に調整（最小注文額対策・
  章15.4.10 で詳述）
- `watchlist.directions: [LONG, SHORT]` を維持
- Arbitrum One 経由で $295 USDC 入金（5/13 16:24 UTC）
- BOT 再起動 → 数時間で複数の隠れバグが発覚（章11.19 / 章22.7 Unified Account 節で詳述）

**secrets の network 切替運用（ファイル差し替え方式）:**

`secrets.enc.yaml` は単一ネットワーク構造（mainnet/testnet 併載ではない）。
切替は **ファイル単位の差し替え**で行う:

```
secrets.enc.testnet.yaml  ← testnet バックアップ
secrets.enc.yaml          ← 現用（mainnet または testnet）
```

testnet に戻す時は PowerShell `Copy-Item` で差し替え。将来的にネスト構造
（`hyperliquid.testnet` / `hyperliquid.mainnet`）への再設計余地はあるが、
コード変更を伴うため別 PR。現状は未着手。

#### 15.4.10 Phase 2 Step 2b で発覚した 3 つの隠れバグと修正履歴

mainnet 実弾運用初日に「4 層通過 5 件 → trades 0 件」事象が発生
（章11.19 / 章22.7 参照）。原因切り分けにより**多層的なバグ**と判明し、3 つの
コミットで段階的に修正:

| commit | 内容 | 章 |
|---|---|---|
| `3df8bea` | `_execute_entry` の silent rejection 2 箇所に logger.warning + send_alert を追加（Level 1+2 可視化） | 11.19 / 19.4 |
| `79a311f` | unified account 対応（spot 残高を perp 担保として読む） | 22.7 (Unified Account 節) |
| `c9b2d50` | `_grouped_status_to_result` の entry slot inner error を raise 統一（Level 3 構造的修正） | 11.19 |

修正後の rejection 可視化マトリクスは章 11.19.3 表参照。

#### 15.4.11 REGIME 層のデカップリング問題（mainnet 実観測データ）

**観察した現象（mainnet Phase 2 Step 2b 再起動後・約 20 時間）:**

BTC が上昇トレンド・ETH が下落トレンドという**デカップリング局面**で、
ETH 自体は下降トレンドにもかかわらず REGIME が BTC 基準で UPTREND の
ため ETH SHORT 経路が一切通らない事象を確認。

**実測値（再起動後 約 20 時間・3,351 SHORT cycle）:**

| 層 | SHORT 通過率 | 備考 |
|---|---|---|
| MOMENTUM | 1.8%（61 件） | ETH 5bar momentum < -0.3% を満たす瞬間 |
| FLOW | 100% | 簡易実装（章11.6.3）で常時 1 |
| SENTIMENT | 56.1% | funding contrarian で ETH bearish 寄り |
| REGIME | **0.0%（0 件）** | 3,351 cycle 全て BTC UPTREND でブロック |
| **4 層 AND** | **0 件** | ↑ REGIME が完全閉鎖のため成立せず |

LONG 側も同じ観察期間で 0 件成立。実トレード件数は **0 件のまま**。

**「per-symbol REGIME ならどれだけ通過するか」の対抗事実シミュレーション:**

DB 上の `signals` を集計し、`SENTIMENT 通過 ∧ MOMENTUM 通過 ∧ REGIME =
'BTC が UPTREND だがその銘柄自身は DOWNTREND'` の条件で、もし REGIME が
銘柄個別の EMA で動いていたら 4 層 AND になっていたであろう cycle 数を
算出:

```
ETH SHORT で「per-symbol REGIME なら通っていたはず」の cycle:
  起動後 5h 時点:  41 件
  起動後 20h 時点: 41 件  ← 直近 15 時間で増分ゼロ
```

→ 取り逃した機会は**最初の 5 時間に集中**しており、その後はゼロ。
20 時間で 41 件、平均 2 件/時 のオーダー。

**結論（中立記録・Phase 4 判断材料）:**

- **REGIME が BTC 基準であること自体は意図的設計**（章4 「マクロ環境
  フィルター」と命名・§15.2 / §20.B で Phase 4 課題と予告済み）
- **デカップル局面での機会損失は実在する** が、定量的には**限定的**
  （20h で 41 cycle・後半 15h は増分ゼロ・相場依存）
- **真のボトルネックは MOMENTUM（1.8%）**。相場の動意が薄い時間が長く
  続くため、REGIME を per-symbol 化しても MOMENTUM で詰まる可能性が高い
- **REGIME 単独修正の費用対効果は低い**。Phase 4 で BTC.D フィルターと
  併せて精緻化するのが妥当（§20.B 既存方針と整合）

per-symbol REGIME に変更する場合の実装規模は **PR 1 本・1-2 日**程度
（C 案: ハイブリッド・非破壊で `MarketSnapshot` に `symbol_ema_trend` を
追加する形）。即着手する判断材料は揃っていないが、選択肢として残しておく。

#### 15.4.12 4 層 AND の機会頻度に関する構造的トレードオフ

mainnet Phase 2 観察で判明した 4 層通過の構造的問題（章15.4.11 を一般化）:

| 方向 | 主ボトルネック | 副ボトルネック | 構造的相性 |
|---|---|---|---|
| LONG | SENTIMENT（funding 過剰 LONG では bullish 出ない） | MOMENTUM | SENTIMENT vs MOMENTUM が逆方向に動きがち |
| SHORT | REGIME（BTC UPTREND でブロック）+ MOMENTUM | FLOW（簡易実装） | REGIME vs SENTIMENT が逆方向に動きがち |

**4 層 AND のトレードオフ:**

- 利点: 質の高いシグナルだけ通る（誤エントリー抑制）
- 代価: 特定の相場の組み合わせでしか 4 層が揃わない → 機会が少ない

これは **バグではなく設計上のトレードオフ**。機会頻度を上げるには:

1. **層単体の閾値緩和**（小手先・効果限定的）
2. **層構造の見直し**（4 層 OR 化、層ごとに必須/任意フラグ、銘柄カテゴリ
   別の閾値など）— Phase 4 以降の設計議論
3. **戦略の追加**（4 層 AND とは別の経路を共存させる・複数戦略の合算）

PR7.6.7 時点では「現状の 4 層 AND は通りにくい設計だが、それは品質
フィルターとして機能している」と中立的に記録する。最終判断は実損益が
出始める Phase 4 以降。
...
**PR D1 (2026-05-16) で実装済み**: ``meta_and_asset_ctxs`` の cycle cache。
``Scheduler.run_cycle_once`` 冒頭の ``invalidate_meta_cache()`` で
明示クリアし、cycle 内では同 API を 1 回しか叩かない。詳細は §20.4.D。
これにより上記 ``get_market_snapshot(BTC)`` / ``get_market_snapshot(ETH)``
や ``get_funding_rate_8h`` / ``get_open_interest`` の内部経路が重複して
HL meta API を叩く問題は構造的に解消。後続の PR D2 (per-symbol REGIME)
と PR D3 (watchlist 拡大) の **前提条件** として位置付けている。

### 19.3 PR7.4-real / SHORT 検証で確定した API コール数とレート制限運用

PR7.4-real（CircuitBreaker 実値化）でさらに +3 calls/cycle、SHORT 検証
（`profile_phase1_short.yaml`）で `attempts` が 2 → 4 に倍増。
testnet 観察で 429 が頻発する組合せが判明したので運用ガイドラインを確立。

**1 cycle あたりの API コール数の推移:**

```
PR7.6 時点                       :  7 calls/cycle
PR7.7 (BTC EMA/ATR)              : +2 calls (15m candle × 2 use cases)
                                 →  9 calls/cycle
PR7.4-real (CircuitBreaker)      : +3 calls (1m × watchlist + BTC 5m)
                                 → 12 calls/cycle
SHORT 検証 (LONG+SHORT directions): +0 calls/cycle
  ※ get_market_snapshot 等は cycle 単位で 1 回。attempts 増は cycle 内の
    entry_flow 呼び出し回数増だが、内部の exchange/repo 呼び出しは attempts
    のたびに走るため実際には倍程度に膨らむケースあり（実測で 429 発生）
```

**testnet 観察での 429 発生パターン:**

| profile | cycle | attempts | 実効 req/min | 結果 |
|---|---|---|---|---|
| phase0 (LONG のみ・FixedSentiment) | 10s | 2 | ~72 | 安定（PR7.7 期間 72h で実証） |
| phase1 (LONG・FundingRate) | 10s | 2 | ~72 | 安定 |
| phase1_short (LONG+SHORT) | **10s** | **4** | ~144 | **約 1 cycle/min で 429**・~11% cycle 失敗 |
| phase1_short (LONG+SHORT) | **20s** | **4** | ~72 | 429 完全消失 |

**運用ガイドライン:**

- 1 分あたり 100 req 未満を目標に `cycle_interval_seconds` を調整する
- 概算式:
  ```
  銘柄数 × 方向数 × (60 / cycle_seconds) × calls/cycle ≦ 100
  ```
- 将来 watchlist 拡大時（4 銘柄 + LONG/SHORT = 8 attempts）は事前に試算
- HL の正式レート制限は 1200 req/min（章22.2）だが、burst で短時間に
  集中すると 429 が出る。**1200 の数値だけで判断せず実機検証する**

**429 発生時の自動回復挙動:**

`evaluate_and_enter` が `ExchangeError` を上げると：
- `entry_flow` 内の `_safe_notify` で alert 通知（`dedup_key="entry_fail:{symbol}:{direction}"`
  により Discord 通知は 5 分窓で抑制・章25.8.3）
- 該当 attempts の `entry_errors` カウント増
- 次サイクルで自動的に再試行 → 多くの場合 1 分以内に自動回復

testnet 観察例:
```
60 秒間 429 burst → ~1 分後に自動回復
Discord 通知は dedup_key で 1 件のみ（連発しない）
DB には errors として全件記録される（後から分析可能）
```

`api_error_rate_5min` が `api_error_rate_threshold`（既定 0.30）を超えると
CircuitBreaker Layer 6 が発動して新規エントリーを止める（章9.15.5）。
testnet の 11% は閾値以下なので発動せず、cycle 失敗のままシステムは継続稼働。

### 19.4 mainnet Phase 2 Step 2b で確定した運用パラメータ

testnet と同じ「LONG+SHORT」構成を mainnet でも実機検証した結果、
**testnet と類似の 429 傾向**が確認された。`cycle_interval_seconds` の
チューニング履歴と現状の決定値:

| 期間 | cycle_interval | 実効 req/min | 状態 |
|---|---|---|---|
| Phase 2 Step 2b 初期 | 20s | ~144 | mainnet でも時間帯で 429 集中発生 |
| Phase 2 Step 2b 調整後 | **30s** | ~96 | 429 は減少するが完全には消えない |

PR7.4-real で導入した `get_candles` 系の追加 API コール（章19.2 / 19.3）が
mainnet でも効いている。429 が残る理由は `meta_and_asset_ctxs` が
cycle ごとに毎回呼ばれているため（章20 で改善候補として記録）。

**429 と 4 層通過のリスク相関:**

429 自体は次サイクルで自動回復するが、**「4 層通過がクラスター的に発生する
局面（例: 暴落初期の数分間）」と 429 が重なるとシグナルを取り逃がす**。
mainnet では 4 層通過自体が稀（章15.4.11 参照）なので顕在化はしていないが、
将来 watchlist 拡大時には注意が必要。

**運用方針（現状）:**

- `cycle_interval_seconds: 30.0` を維持
- 根本解決は `meta_and_asset_ctxs` キャッシュ実装（章20.D）

#### 19.4.1 DB の mainnet/testnet 分離

`profile_phase2.yaml` で `storage.db_path: data/hl_bot_mainnet.db` に変更
し、testnet データ（`data/hl_bot.db`）と物理的に分離した。これにより:

- mainnet 実弾レコードと testnet 観察レコードが混在しない
- `analyze_phase0.py --db data/hl_bot_mainnet.db` で mainnet 分のみ集計
- 万一の操作ミスで testnet DB を壊しても mainnet は無傷
- BOT 起動時に `SQLiteRepository.initialize()` が schema を冪等適用するので
  新規 DB ファイル作成は自動

#### 19.4.2 最小注文額（HL ~$10 notional）と position_size_pct の関係

HL には最小注文額（notional 下限・実測で約 $10）がある。`position_size_pct`
を小さくしすぎると、4 層通過しても `calculate_position_size` が
`size_too_small_after_rounding` で発注に至らない。

**Phase 2 Step 2b 起動時の試算:**

```
balance ($295) × position_size_pct (0.005) × leverage (3) = $4.4
                                                    ↑ HL 最小未満
→ size_coins ≈ 0 で skip
→ Step 2b 初日の Phase 2 BOT が 4 層通過してもこれで止まる可能性

balance ($295) × position_size_pct (0.02) × leverage (3) = $17.7
                                                    ↑ HL 最小クリア
→ Step 2b 採用値
```

入金額・leverage・最小注文額の関係を踏まえて `position_size_pct` を
設定する必要がある。`profile_phase2.yaml` の現行値 `0.02` は $200〜500
の入金規模を想定。
...
### 20.4 mainnet 観察で優先度更新された改善候補

PR7.6.7 時点で改善候補の優先度を実観察データに基づいて更新:

#### 20.4.D `meta_and_asset_ctxs` キャッシュ（昇格: 中→**中の上**）

**現状の問題:**
- `meta_and_asset_ctxs` は cycle ごとに毎回 API 呼び出しされる
  （規模 100〜200 行レスポンス）
- attempts=4 構成で 429 自然発生の主因
- `cycle_interval=30s` で当座は凌げているが、根本解決ではない

**実装イメージ:**
- 1 cycle 内で同じ呼び出しは共有（`get_market_snapshot(BTC)` と
  `get_market_snapshot(ETH)` が同じ `meta_and_asset_ctxs` を使う）
- cycle 境界でキャッシュ無効化
- 規模感: PR 1 本・100〜200 行

**昇格理由:**
- mainnet 移行後の API コール状況で根本解決として価値が明確化
- `cycle_interval` を短く保てれば、4 層通過クラスター期の取り逃しが減る

**実装完了（PR D1, 2026-05-16）:**
- ``HyperLiquidClient._fetch_meta_and_ctxs`` がキャッシュ対応。
  ``_meta_cache: tuple[meta, asset_ctxs, monotonic_fetched_at] | None``
  を保持し、TTL ``_META_CACHE_TTL_SECONDS = 1.0`` 内なら再利用。
- ``HyperLiquidClient.invalidate_meta_cache()`` を ``ExchangeProtocol``
  に追加。``Scheduler.run_cycle_once`` の冒頭で必ず呼び、明示的に
  キャッシュをクリアする（TTL は明示クリアの取りこぼし時の safety net）。
- 既存 callsite（``get_symbols`` / ``get_funding_rate_8h`` /
  ``get_open_interest`` / ``get_market_snapshot``）は変更不要で透過的に
  恩恵を受ける。
- ``cycle_interval=30s`` / attempts=4 構成で 1 cycle あたり HL meta API
  呼び出し: **N→1** に縮約。
- PR D2 (per-symbol REGIME: 銘柄ごと candle 取得が増える) と
  PR D3 (watchlist 拡大: HYPE 等で銘柄数増) の **前提条件**として導入。

#### 20.4.B BTC.D / per-symbol REGIME（据え置き: Phase 4）

§20.B 既存記述の通り Phase 4 案件として維持。Phase 2 Step 2b 観察
（章15.4.11）で「per-symbol REGIME の取り逃しは 20h で 41 cycle・
...
#### UTC 00:00 ローソク足取得のフォールバック（PR7.x-fix）

`_get_utc_day_open_price` は当日 UTC 00:00 の 1h 足の open を取得して
MOMENTUM の補助フィルタ（`current_price > utc_open_price`）に使う。
testnet 観察で **HL 側が一時的に 00:00 の 1h 足を未配信になる瞬間**を確認
（48h で 6 回、連続 4 cycle 続くパターンあり）。当初は ExchangeError で
cycle を全停止していたが、PR7.x-fix で fallback 方式に変更：

```python
async def _get_utc_day_open_price(
    self, symbol: str, fallback_price: Decimal
) -> Decimal:
    """ローソク取得失敗 / 空応答時は warning ログ + fallback_price 返却。
    fallback_price は呼び出し側で既に取得済みの current_price (markPx) を渡す。
    """
    try:
        response = await asyncio.to_thread(
        ...
        logger.warning("Failed to fetch UTC 00:00 candle for %s: %s, "
                       "falling back to current_price %s", symbol, e, fallback_price)
        return fallback_price
    if not response:
        logger.warning("No UTC 00:00 candle for %s, falling back to %s",
                       symbol, fallback_price)
        return fallback_price
    return Decimal(str(response[0]["o"]))
```

**設計判断: fallback_price を caller から渡す方式**:
- HyperLiquidClient の current_price ソースは `markPx`（`metaAndAssetCtxs`）
- caller (`get_market_snapshot`) は既に line 1 で markPx を取得済み
- 別 API（例: `all_mids`）の再呼び出しは新しい failure mode を作るだけ
- 既存パターン尊重 + 追加コール 0 で最も安全

**utc_open_price の用途と fallback の安全性:**
- 用途: MOMENTUM 補助フィルタ。`current_price > utc_open_price` で LONG 加点
- fallback 時の挙動: `current_price > current_price` = **常に False**
- 結果: LONG 判定が安全側（不採用方向）に振れるだけ → 運用上問題なし

メインループの継続稼働ポリシー（章19）と整合: API 一時障害で BOT 全停止は
本番運用で許容不能、可能な限り部分機能で継続する。
...
#### Unified Account モード（mainnet で検出・PR 79a311f で対応）

HL のアカウント抽象化機能で、**spot USDC が perp 取引の担保として
自動的に使われる** cross-margin モード。`spot ↔ perp` の手動内部
転送が不要になる代わりに、残高取得 API の挙動が従来と異なる。

**検出方法:**
```python
info.query_user_abstraction_state(master_address)
# → "unifiedAccount" / "splitAccount" / None など
```

**各 API の見え方（実機検証済み・2026-05-14）:**

| API | unifiedAccount 時の挙動 |
|---|---|
| `info.user_state(master).marginSummary.accountValue` | **`"0.0"`** ← perp 直接保有のみ。spot 担保は反映されない |
| `info.user_state(master).crossMarginSummary.accountValue` | 同上 `"0.0"` |
| `info.user_state(master).withdrawable` | 同上 `"0.0"` |
| `info.user_state(master).assetPositions` | `[]`（perp ポジションなしの場合） |
| `info.spot_user_state(master).balances` | `[{coin: "USDC", total: "295.0", ...}, ...]` ← **ここに実額** |
| `info.spot_user_state(master).tokenToAvailableAfterMaintenance` | `[[0, "295.0"]]` ← **maintenance margin 控除後の実利用可能額（最精確）** |

**実発注時の挙動:**

spot USDC は **自動的に perp 取引の担保として扱われる**。手動転送せずに
直接 perp 銘柄に発注可能。実機検証で ALO sell ALO @ markPx×1.15 を
notional $14.97 で発注 → `OrderResult(success=True, order_id=...)` を確認
（spot $295 のみで perp 0 だが受理）。

**balance 取得実装の方針:**

`info.user_state` のみ参照すると unifiedAccount で常時 $0 が返り、BOT が
size=0 で silent skip する事故になる。実装は:

1. 初回 `query_user_abstraction_state` を呼んで結果をキャッシュ
2. `unifiedAccount` なら `spot_user_state` を参照
   - `tokenToAvailableAfterMaintenance[token=0=USDC]` を優先
   - 無ければ `balances[USDC].total` にフォールバック
3. それ以外（splitAccount 等）なら従来通り `marginSummary.accountValue`
4. abstraction state 取得失敗時は `splitAccount` 扱い（fail-safe）
   - 理由: unified 誤判定で split ユーザーが spot 残高基準で size 計算
     → 実発注で margin 不足拒否を踏むため

**現状の制約（PR 79a311f 時点）:**

- 抽象状態キャッシュ寿命 = BOT ライフタイム。稼働中に HL UI で
  ユーザーがモード切替してもキャッシュは更新されない（再起動必要）
- `token_id=0` (USDC) のみ対応。他トークン担保（USDE / USDT0 / USDH）は
  読まない
- API コール: 起動時 +1（abstraction state）。unified では user_state
  の代わりに spot_user_state を呼ぶので **総数は不変**
