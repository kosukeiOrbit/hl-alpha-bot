...
② FLOW（HyperLiquid大口検出）
   直近5分の買い約定額 / 売り約定額 > 1.5
   AND 大口約定（>$50k）が買い優勢
...
**起動時の対処（Bot再起動時にUTC 00:00を跨いだ場合）：**
- DB未登録なら、HL APIの`candleSnapshot`（1m足）から該当時刻の終値を遡って取得
- それでも取れない場合は`prevDayPx`で代用してログに警告
...
class EntryFlow:
    ...
    async def _build_snapshot(self, symbol: str) -> MarketSnapshot:
        ...
        utc_open = await self.repo.get_utc_open_price(symbol)
        # ... 全データを集約
